...
# 4-skip 6 
# keyword = "math"
# ['math', 'social','math', 'marketing', design', 'web', 'cyber', 'program', 'platform', 'map', 'intelligence', 'knowledge', 'graph', 'probability']
# 'digital', 'electronic', 'architecture', 'infrastructure', 
keywords = [ 'social', 'marketing', 'design', 'web', 'cyber', 'program', 'platform', 'map', 'intelligence', 'knowledge', 'graph', 'probability', \
'digital', 'electronic', 'architecture', 'infrastructure', 'digital', 'electronic', 'architecture', 'infrastructure', 'program', 'platform', 'map', 'intelligence', 'entrepreneurship','cyber', 'knowledge', 'graph', 'probability', "engineer"]
for keyword in keywords:

    for j in xrange(2, 5):
        url = "https://www.coursera.org/courses?languages=en&query=" + keyword + "&start=" + str(j * 20)
        print url
        ...
        url = soup.select_one("script[type=application/ld+json]")
        if len(url) == 0:
            continue
        if len(url) != 0:
            url_list = json.loads(url.text)
            ...
            for i in range(len(urls)):
                print urls[i]
                browser.set_page_load_timeout(200000000) 
                erroe = browser.get(urls[i])
                print erroe
                browser.get(urls[i])
                time.sleep(20)
                courseObject = page[i]
                ...
                newpage.append(courseObject)
                print newpage
        with open('Coursera_'+ keyword + str(j) + '.json', 'a') as f:
            json.dump(newpage, f)

    content += newpage
with open('Coursera_'+ keyword + '_content' + '.json', 'a') as f:
    json.dump(content, f)
