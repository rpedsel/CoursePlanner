...
_x_query = {
    ...
    "provenance": ["//span[@class = 'text-light offering-partner-names']/span",
                   "//div[@class = 'text-light offering-partner-names']/span"]
}

content = []
id = 0
for i in range(0, 42):
    url = "https://www.coursera.org/courses?languages=en&query=data&start=" + str(i * 20)
    ...
    for i in range(len(urls)):
        ...
        if 'specializations' in urls[i]:
            ...
            for tag in CourseTags:
                ...
                # coursenames = [item.text.encode('ascii','ignore') for item in cousenames]
            ...
            # coursename =  browser.find_elements_by_xpath('//h1[@class="title display-3-text"]')
            # description = description.text.encode('ascii','ignore')
    content += page
browser.quit()

with open('Coursera_data.json', 'a') as f:
    json.dump(content, f)
